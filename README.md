...
**Raspberry Pi Pico 2 W** を推奨します（無印 Pico 2 でも動作しますが、WiFi チップがないため
NTP による時刻同期のみ利用できません。他の機能に制限はありません）。
...
- **モダンな表示サポート**: ILI9341 (320x240) / ST7796 (480x320) の TFT LCD に対応（`pb1000.ini` で選択）。タッチインターフェース (XPT2046) を統合。
- **外部キーボード対応**: HID USB キーボード (Host モード) による入力をサポート。
...
- **HDMI ミラー出力**: 2台目の Pico 2 (RP2350) を HSTX 経由の受信機として接続し、実機LCDの表示内容をHDMI出力にミラーリング可能（任意、詳細は [Hardware Guide](doc/hardware_guide.md) 参照）。
- **エミュレータメニュー**: F7キーまたは画面下部のファンクションキーバーから開ける設定・状態管理UI（RAM保存、ディスク差し替え、フック状態表示など）。保存済みRAMの復元は起動時のプロファイル選択画面から行います。
