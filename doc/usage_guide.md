...
1. Pico 2 に電源を入れる。
2. **プロファイル選択 UI** が表示される。SD カード上の `/sd/profiles/` を走査してリストアップ。
   - 上下キーまたはタッチでプロファイルを選択し EXE / タッチで決定。
   - タイムアウト（デフォルト 5 秒）で `default_profile` の設定値が自動選択される。
...
| PC キー | 機能 | 備考 |
| :--- | :--- | :--- |
| **NumLock** | リセット | ハードウェアリセット（PC=0x0001） |
...
- **MENU / CAL / LCKEY**: それぞれ F5 / F6 / F7 にマッピング
