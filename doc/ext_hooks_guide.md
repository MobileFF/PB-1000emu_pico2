...
エミュレータに用意されている汎用サブルーチンフックのサンプルコード一覧と BASIC からの使い方。

**標準搭載のサブルーチンフックは存在しない。** 本書に登場するフックはすべて**サンプルコード**で
あり、BASIC プログラムから明示的に `CALL` して初めて動作する。エミュレータ本体の基本動作
(CPU・LCD・キーボード等)には一切影響しない。どのフックも SDカード（`/sd/ext/`）またはフラッシュ
（`/ext/`）にファイルを**配置するかどうか**をユーザー自身が選べるため、使うかどうか、どのフックを
使うかは完全にユーザーの自由である。

ハードウェア専用フック（DHT20 温湿度センサー等）もサンプルの一つとして本書で紹介する。  
フックの仕組みや独自フックの作り方は [extension_api.md](extension_api.md) を参照。

> **注意**: このリポジトリの配布状態では `bank_loader.py`（`mp/ext/`）はあらかじめ配置済みのため
> 何もしなくても動作するが、`ram_test.py`・`dht20.py` は `sample/mp/ext/` に置かれたサンプルで
> あり、`mp/ext/`（または `/sd/ext/`）へコピーしない限り実機では自動ロードされない。両者に
> 機能上の違いはなく、単に配置済みかどうかだけの違いである。
> 以下の CALL &H5E41/51/61/71（ram_test）・&H5E10（dht20）を使うには先にコピーが必要。
...
## ram_test.py（サンプル、要コピー）

BANK2/3 のバンク切替フックとRAMテストを提供する。  
バンク切替フックは POKE/PEEK で直接バンクRAMにアクセスする際に使用する。

`sample/mp/ext/ram_test.py` に置かれたサンプルモジュールであり、`mp/ext/` に配置済みというわけ
...
I2C 接続の DHT20 温湿度センサーを読み取るハードウェア専用フック。  
`sample/mp/ext/dht20.py` に置かれたサンプルモジュールであり、`mp/ext/`（または `/sd/ext/`）へ
コピーしない限り実機では自動ロードされない。対応する BASIC サンプルは `sample/basic/DHT20.BAS`。
...
### CALL &H5E10 — DHT20 読み取り

#### 出力（CALL 後に PEEK）

| オフセット | 内容 |
|---|---|
| `&H5F00` | 結果コード: `0x00`=OK / `0xFF`=エラー（センサー未応答・測定未完了等） |
| `&H5F01` | 温度×10 上位バイト（符号付き16bit、big-endian） |
| `&H5F02` | 温度×10 下位バイト（例: `&H00EB`=235 → 23.5℃） |
| `&H5F03` | 湿度×10 上位バイト（符号なし16bit、big-endian） |
| `&H5F04` | 湿度×10 下位バイト（例: `&H025D`=605 → 60.5%） |
...
| アドレス | モジュール | 機能 |
|---|---|---|
| `&H5E10` | dht20（サンプル、要コピー） | DHT20 温湿度センサー読み取り |
| `&H5E41` | ram_test（サンプル、要コピー） | データバンク → BANK2 |
| `&H5E51` | ram_test（サンプル、要コピー） | データバンク → BANK3 |
| `&H5E61` | ram_test（サンプル、要コピー） | データバンク → 元に戻す |
| `&H5E71` | ram_test（サンプル、要コピー） | BANK2/3 RAM 全テスト |
| `&H5E81` | bank_loader（サンプル、配置済み） | SDカードファイル → バンクRAM |
| `&H5E91` | bank_loader（サンプル、配置済み） | FDDファイル → バンクRAM |
